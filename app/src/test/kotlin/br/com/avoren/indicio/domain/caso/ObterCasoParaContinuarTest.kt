...
class ObterCasoParaContinuarTest {
    ...
    private fun usoDe(
        ...
        catalogo: Catalogo = Catalogo(1, listOf(resumo())),
    ...
    fun `nao oferece caso removido do catalogo`() = runTest {
        val resultado = usoDe(progresso(), catalogo = Catalogo(1, emptyList())).invoke().first()
    ...
    fun `nao oferece caso que deixou de estar disponivel`() = runTest {
        val catalogo = Catalogo(1, listOf(resumo(disponivel = false)))
